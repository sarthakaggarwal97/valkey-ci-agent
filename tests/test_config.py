...
from scripts.common.config import BotConfig, ProjectContext, load_config
...
def test_config_round_trip_all_fields(config_data: dict) -> None:
    ...
    try:
        ...
        assert cfg.max_open_prs == config_data["limits"]["max_open_prs"]
...
def test_missing_config_returns_defaults() -> None:
    ...
    assert cfg.max_open_prs == defaults.max_open_prs
...
def test_valid_fields_preserved_with_unrecognized_ignored(
    ...
    **Validates: Requirements 8.4**
    ...
    try:
        ...
        assert cfg.max_open_prs == limits_data.get("max_open_prs", defaults.max_open_prs)
