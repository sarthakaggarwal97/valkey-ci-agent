...
def test_verify_ignores_bullets_in_non_sweep_commit_bodies(tmp_path, monkeypatch) -> None:
    """Ordinary commits legitimately carry ``* ... (#N)`` bullets (e.g. a
    squashed feature PR listing its own commits, or a changelog-style body).
    Those reference unrelated PRs and must never verify a board item."""
    import subprocess
    ...
    body = (
        "Refactor connection handling (#5000)\n\n"
        "* extract pooling helper (#4990)\n"
        "* tighten timeouts (#4991)\n"
    )
    git("init", "-q")
    (repo / "f").write_text("1")
    git("add", "f")
    ...
    # Only the commit's own trailing (#5000) counts (subject tier); the
    # body bullets of a non-sweep commit never do.
    assert present == {5000}
