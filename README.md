...
1. **Prepare Release** authorizes the dispatcher against
...
2. The narrow **Refresh Release Progress** watcher derives status from live
   ...
   neither its body nor its checkboxes authorize a transition.
3. **Publish Release** requires the exact SHA to remain branch HEAD and to be
   ...
   release-notes section, every configured `ci.yml` check on that SHA, tag
...
commit and requires any supplied SHA to match, so no Valkey-side relay change
is needed. It retains the complete production fan-out: archives, RPM/DEB
...
- `release-control` must allow only the exact default branch. It stores
  ...
  credentials. The control App reads Valkey and production workflow status,
...
- `release` must allow only the exact default branch, require a reviewer,
  ...
  `VALKEY_RELEASE_PUBLISH_APP_PRIVATE_KEY`. The publication App is the sole
...
For a maintainer, the normal path is: start one real **Prepare Release** run,
