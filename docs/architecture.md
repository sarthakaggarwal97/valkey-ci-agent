...
```text
ci_fix/main.py (workflow_dispatch or issue_comment event)
  ...
  -> pipeline.run_ci_fix(...)
       ...
       review.run_fix_loop       -> apply (edit-only AI)
                                    -> runner.run_verification_command (code
                                       runs the AI-proposed command in a
                                       sanitized subprocess; exit code is the
                                       verdict)
                                    -> review_fix (skeptic AI)
                                    repeat on feedback; success needs the test
                                    to pass AND the review to approve
       push.commit_and_push_fix  -> extract approved patch
                                    -> apply in a fresh trusted clone
                                    -> DCO commit, push to the PR's own
...
AI proposes (which test, how to fix, how to run it, whether the fix is sound)
and code disposes (runs the command, owns pass/fail, performs the push). The AI
subprocess never executes a command and never touches the remote. The checkout
that runs untrusted test code never receives push credentials; publishing
happens by applying the approved patch in a fresh clone at the gated SHA.
...
- `scripts/ci_fix/main.py` — workflow_dispatch / issue_comment event entry point
- `scripts/ci_fix/gate.py` — command parsing, fail-closed team auth, SHA-bound run gating
- `scripts/ci_fix/diagnose.py` — read-only AI diagnosis into a structured proposal
- `scripts/ci_fix/apply.py` — edit-only AI fix application
- `scripts/ci_fix/runner.py` — sanitized command execution that owns the verdict
- `scripts/ci_fix/review.py` — skeptic review and the apply/run/review loop
- `scripts/ci_fix/push.py` — patch handoff, DCO commit, namespace-restricted push
- `scripts/ci_fix/comment.py` — render the outcome into a PR comment
- `scripts/ci_fix/pipeline.py` — top-level orchestration
