...
1. **Command shape** — the comment must be the fix command with a CI run link
   on the *same* repository as the PR.
2. **Authorization** — the commenter must be an active member of a configured
   GitHub team (``valkey-io/contributors``). A failed or negative membership
   read is a refusal, never a fallback to a looser check.
3. **SHA-bound run gating** — the failed run's ``head_repo``/``head_branch``
   must match the PR head, and the PR head SHA must still equal the SHA the
   run was built from. If the branch moved, the log no longer describes the
   code, so we refuse rather than fix a stale failure.
...
# "@valkeyrie-bot fix <url>" optionally followed by a free-text hint.
_COMMAND_RE = re.compile(
    r"@valkeyrie-bot\s+fix\s+(?P<url>\S+)(?:\s+(?P<hint>.*))?",
...
def build_fix_request(
    ...
    *,
    ...
    return FixRequest(
        ...
        pr_number=pr_number,
        head_repo_full_name=pr_head_repo or pr_repo_full_name,
