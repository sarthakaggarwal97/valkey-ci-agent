...
The apply prompt restates the immutable guardrail: fix test scaffolding, never
weaken the assertion under test. ``feedback`` carries the reason a previous
attempt was rejected (a failing test run or a skeptic rejection) so the agent
...
_PROMPT_TEMPLATE = """\
You are fixing a single failing test on a release branch. A diagnosis has
...
## Hard rules
- Edit ONLY what is needed to fix this one test.
- Fix test SCAFFOLDING (payloads, version bytes, helpers, iteration counts,
  setup). NEVER weaken, loosen, or delete the assertion the test exists to
  verify. If the only way to make the test pass is to weaken its assertion,
  STOP and make no edits — that means the failure is a real product bug, not a
  test bug.
- Do not run builds, tests, git, or any commands. Code will build and run the
  test after you edit.
...
_PORT_PLAN = (
    ...
    "change to the failing test on this branch (commit {commit}). Adapt it to "
    "this branch's APIs if needed; do not pull in unrelated changes."
)

_AUTHOR_PLAN = (
    "Write a minimal scaffolding fix in the failing test, per the root cause. "
    "{reasoning}"
)
