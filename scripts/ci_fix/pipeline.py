...
Code owns environment selection end to end: the AI only hints which job failed;
code confirms that against the real failed jobs and classifies that job's
workflow environment. The AI never chooses where verification runs.
"""
...
def _verify_once_and_push(
    ...
    *, verifier: VerifyBackend | None, git_env: dict[str, str], push_func: Push,
    ...
    except Exception as exc:  # noqa: BLE001 - every outcome must become a comment
        logger.exception("macOS verification raised unexpectedly")
        return FixOutcome(
            kind=OutcomeKind.FAILED,
            summary=f"Internal error in macOS verification: {exc}",
...
def _classify_failing_job(repo_dir: Path, failing_job: str) -> JobEnvironment:
    ...
    for path in sorted(workflows.glob("*.y*ml")):
        env = classify_job_environment(path.read_text(encoding="utf-8", errors="replace"), base)
