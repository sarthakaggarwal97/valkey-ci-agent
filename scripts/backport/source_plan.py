...
class SourceChangePlan:
    ...
    ``commit_sha`` is the single authoritative commit carrying the pull
    request's complete change: the merge commit for ``merge``, the aggregate
    commit for ``squash``, or the pull request's lone commit for ``single``.
...
def plan_source_change(
    ...
    if not merge_commit_sha:
        ...
        return SourceChangePlan(strategy="single", commit_sha=source_commits[0])
    ...
    if len(merge_parents) > 1:
        return SourceChangePlan(strategy="merge", commit_sha=merge_commit_sha)
    ...
    if len(source_commits) <= 1:
        return SourceChangePlan(strategy="single", commit_sha=merge_commit_sha)
    ...
    if source_patch_id and source_patch_id == merge_patch_id:
        return SourceChangePlan(strategy="squash", commit_sha=merge_commit_sha)
