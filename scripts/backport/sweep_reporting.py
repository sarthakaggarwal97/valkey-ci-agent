...
def build_pr_body(
    ...
    *,
    ...
    lines = [
        f"# Backport sweep for "
        f"{escape_text(result.target_branch, multiline=False)}",
        "",
    ...
    if applied:
        ...
        for r in applied:
            ...
            if url and _is_ai_resolved(r):
                detail = markdown_link(r.detail, url)
            else:
...
def build_summary(results: list[BranchSweepResult]) -> str:
    ...
    for r in results:
        ...
        suffix = f" -- {markdown_link('PR', r.pr_url)}" if r.pr_url else ""
        if r.error:
            suffix += f" -- error: {escape_text(r.error, max_bytes=2048)}"
        lines.append(
            f"- {inline_code(r.target_branch)}: "
            f"{applied}/{r.candidates_found} applied" + suffix
        )
    return bounded_body("\n".join(lines))
