...
def verify_prs_on_branch(
    ...
    * a backport commit on the branch lists the PR in an ``## Applied`` table
    ...
    All signals come from one full-message history scan. Trailer and subject
    matching are structural; the body matching reads only the ``## Applied``
    section, so a stray ``(#N)`` reference in a ``## Needs attention`` row or
    ...
    with GitAuth(token, prefix="mark-done-git-askpass-") as git_auth:
        ...
        with tempfile.TemporaryDirectory(prefix="mark-done-verify-") as tmp:
            ...
            applied = _applied_prs_from_commit_messages(repo_dir)
...
def _applied_prs_from_commit_messages(repo_dir: str) -> set[int]:
    """Source PR numbers recorded anywhere in full backport commit messages.

    One ``git log -z --format=%B`` pass covers every signal: squash/merge
    subjects, terminal ``Backport-Source-PR`` trailers, and the structured
    ``## Applied`` tables of squash-merged sweep batches. Only that section's
    table cells are read, so a ``(#N)`` in a later ``## Needs attention`` row
    or in prose is never treated as applied.
    """
