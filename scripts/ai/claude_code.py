...
_SANDBOX_RESOLV_CONF = Path("/etc/resolv.conf")
...
def _sandboxed_command(
    ...
    *,
    ...
    resolver_target = _SANDBOX_RESOLV_CONF.resolve(strict=True)
    if not any(
        resolver_target.is_relative_to(Path(runtime_root))
        for runtime_root in _SANDBOX_RUNTIME_ROOTS
    ):
        # systemd commonly places resolv.conf under /run. Mount only that
        # file instead of exposing the rest of the runner's runtime state.
