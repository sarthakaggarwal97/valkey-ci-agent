...
def parse_release_branch(branch: str) -> tuple[int, int]:
    ...
    if not m:
        raise ValueError(
            f"not a release branch: {branch!r} (expected MAJOR.MINOR, e.g. '9.1')"
        )
...
def derive_version(branch: str, intent: ReleaseIntent, tags: Iterable[str]) -> DerivedRelease:
    ...
    if intent is ReleaseIntent.PATCH:
        if not finals:
            raise ValueError(
                f"no final release exists on {branch}; a patch requires an initial "
                f"release (use intent 'rc' or 'ga')"
            )
