...
def tag_ruleset_protected(repo: Any, tag: str) -> TagRulesetVerdict:
    ...
    try:
        ...
        for entry in listing or []:
            ...
            if not (
                {"creation", "deletion"} <= rule_types and ("update" in rule_types or "non_fast_forward" in rule_types)
            ):
                continue
            if "bypass_actors" not in ruleset:
                # GitHub deliberately hides this field unless the token can
                # write rulesets. Do not grant that dangerous permission just
                # for observability: immutability is still verifiable, and the
                # publication App must prove its bypass by creating the tag.
                return TagRulesetVerdict(True, None)
            ids: list[int] = []
